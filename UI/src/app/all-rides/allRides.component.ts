...
export class AllRidesComponent implements OnInit {
  ...
  displayedColumns: string[] = ['source', 'destination','totalBookingCost', 'status' , 'driver', 'rating'];
  ...
  clearSearches(){
    if(JSON.parse(localStorage.getItem("login")).role ===  'admin'){
      ...
      this.userRole = JSON.parse(localStorage.getItem("login")).usertype;
      if(this.userRole === "customer"){
        this.user = "Driver";
        this.http.getAllRides({customername:this.customerName}).subscribe(
          (response)=>{
            console.log(response.json())
            this.allRidesList = response.json();
            this.allRidesList.map(function(d) {
        ...
        this.user = "Customer";
        this.http.getAllRidesForDriver({customername:this.customerName}).subscribe(
          (response)=>{
            console.log(response.json())
            this.allRidesList = response.json();
            this.allRidesList.map(function(d) {
  ...
  rideSearch(driverName){
    this.http.searchBasedOnDriver({customerName: this.customerName, driverName:driverName}).subscribe(
      (response)=>{
        console.log(response.json());
